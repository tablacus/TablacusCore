...
VOID teSetDarkMode(HWND hwnd)
{
	...
	if (_SetWindowCompositionAttribute) {
		...
		DwmSetWindowAttribute(hwnd, 19, &g_bDarkMode, sizeof(g_bDarkMode));
	}
}
...
LRESULT CALLBACK TEDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	try {
		switch (msg) {
		case WM_INITDIALOG:
			if (JS_IsObject(g_pMBText)) {
				for (int i = 1; i < 12; ++i) {
					HWND hButton = GetDlgItem(hwnd, i);
					JSValue val = JS_GetPropertyUint32(g_ctx, g_pMBText, i);
					if (JS_IsString(val)) {
						const char* text_utf8 = JS_ToCString(g_ctx, val);
						std::wstring text = Utf8ToWide(text_utf8);
						SetDlgItemText(hwnd, i, text.c_str());
						JS_FreeCString(g_ctx, text_utf8);
					}
...
/*					if (teGetIDListFromObject(pSB, &pidlParent)) {
		...
		LRESULT lResult = DarkProc(hwnd, msg, wParam, lParam);
		if (lResult != 1) {
			return lResult;
		}
		if (msg == WM_PAINT) {
